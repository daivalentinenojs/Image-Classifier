...
def test(test_loader, model):
    ...
    for aug in range(nb_aug):
        ...
        for i, (images, filepath) in enumerate(test_loader):
            ...
            # rgb_prob = smax_out.data[0]
            # gray_prob = smax_out.data[1]
            # sketch_prob = smax_out.data[2]
            ...
            # if (gray_prob > rgb_prob) and (gray_prob > sketch_prob):
            #     prob = gray_prob

            # if (sketch_prob > rgb_prob) and (sketch_prob > gray_prob):
            #     prob = sketch_prob            
    ...
    # average_prob = total_y_prob / 1000
    open('submissions/average_prob_cuhk.txt', 'w').close()

    # Save to text file
    d_text_file = open("submissions/average_prob_cuhk.txt", "a")
    n = d_text_file.write("Sketch Probability = " + str((total_y_prob/nb_aug)/1000) + '\n' + "Grayscale Probability = " + str((1 - (total_y_prob/nb_aug)/1000)))
    d_text_file.close()

    sub_fn = submission_path + '{0}_Epoch_{1}_Clip_{2}_Runs'.format(epochs, clip, nb_runs)
